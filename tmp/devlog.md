...
/
├── app/
│   ├── main.py            # 엔트리포인트 (GStreamer + VLM 추론 + FCM)
│   ├── server.py          # App HTTP 서버 (MJPEG, SSE, 프롬프트, PTZ)
│   ├── state.py           # 파이프라인 ↔ HTTP 서버 공유 상태
│   ├── camera.py          # 카메라 설정 관리 (영속화 + MediaMTX API + PTZ 설정)
│   ├── ptz.py             # ONVIF PTZ 제어
│   └── hardware.py        # Jetson 하드웨어 모니터
├── web/
│   ├── docker-compose.yml # 웹 대시보드 전용 (cd web && docker compose up -d)
│   ├── src/
...
│   ├── vite.config.js
│   └── package.json
├── config/                # cam_profile.json, ptz 홈, mediamtx.yml (런타임 설정)
├── data/
│   └── cam/{name}/        # 카메라별 클립 데이터 (*.mp4)
├── docker/                # Dockerfile (app, api, web)
...
| Web (web/) | 5173 | 웹 대시보드 — Vue 3 + Vite SPA (`cd web && docker compose up -d`로 별도 실행) |
...
- 카메라 자격증명 제로 하드코딩 — 프론트엔드 입력 → 파일 영속화 → MediaMTX API + PTZ 모듈 런타임 적용
- 설정/데이터 분리 — 설정(`config/`), 데이터(`data/`), 코드(`app/`, `api/`)를 명확히 분리. 카메라 프로파일(연결 정보 + PTZ 홈)은 `config/cam_profile.json` 단일 파일로 통합
...
- [x] debug_server.py 모듈 분리 (ptz / hardware / state / server / dashboard.html)
- [x] 카메라 자격증명 동적 관리 — 하드코딩 제거, 프론트엔드 입력 + 파일 영속화 + MediaMTX API 연동
- [ ] 프론트엔드 Vue/Vite 전환 — dashboard.html을 별도 컨테이너의 Vue 3 (Composition API) SPA로 대체. 전환 완료 시 `dashboard.html` 삭제, `server.py`에서 HTML 서빙(`GET /`) 제거 → `server.py`는 순수 API 서버로만 유지
...
| 컴포넌트 | 역할 |
|---|---|
| `camera.py` | 카메라 설정 영속화 (`/app/camera.json`), PTZ configure, MediaMTX REST API PATCH |
...
| `GET /camera` | 저장된 카메라 설정 조회 |
| `POST /camera` | 카메라 설정 적용 (PTZ + MediaMTX RTSP 소스 동적 설정) |
...
### 260401 — 카메라 자격증명 동적 관리 (v2.0)

#### 하드코딩 자격증명 제거

`ptz.py`의 ONVIF URL/사용자명/비밀번호, `mediamtx.yml`의 RTSP 소스 URL에 평문으로 하드코딩되어 있던 카메라 자격증명을 완전 제거.

#### `app/camera.py` — 카메라 설정 관리 모듈 (신규)

| 함수 | 역할 |
|---|---|
| `load()` | `/app/camera.json`에서 저장된 설정 로드 |
| `save(config)` | 설정을 JSON 파일로 영속화 |
| `apply(config)` | 저장 + PTZ `configure()` + MediaMTX API PATCH 오케스트레이션 |
| `startup_apply()` | 시작 시 저장된 설정 자동 적용 (MediaMTX 재시도 10회, 지수 백오프) |
...
#### MediaMTX REST API 연동

`mediamtx.yml`에 `api: yes` / `apiAddress: :9997` 추가. App 컨테이너가 `PATCH /v3/config/paths/patch/live`로 RTSP 소스를 런타임에 설정. 컨테이너 재시작 불요.

비밀번호 특수문자는 `urllib.parse.quote(safe='')` 처리.

#### `app/ptz.py` — 동적 설정 전환

하드코딩 상수 → `configure(url, user, password)` 함수로 교체. 미설정 상태에서는 모든 ONVIF 명령이 안전하게 무시됨 (early return guard).

#### 프론트엔드 CameraPanel

`web/src/components/CameraPanel.vue` + `web/src/composables/useCamera.js` 신규 추가. 접이식 패널에 IP, 포트, 사용자명, 비밀번호, 스트림 경로 입력 폼. 적용 시 `POST /camera` 호출.

#### 시작 시퀀스 변경

`main.py` 시작 → 웹서버 → `camera.startup_apply()` (백그라운드 스레드) → `camera_ready.wait(60s)` → GStreamer 파이프라인 시작. 저장된 설정이 없으면 타임아웃 후 파이프라인 시작 (프론트엔드에서 설정 가능).
