...
| API 서버 컨테이너 | 기기 토큰 등록, 이벤트 이력 조회, 영상 클립 제공 | REST API |
| DB 컨테이너 | 이벤트 기록, 기기 토큰 저장 | SQLite 내장 or PostgreSQL |

### 4.2 파이프라인 아키텍처 초안

> **구조 확정 (OI-05)**: 디코딩 전 tee 분기 + Branch A 패스스루 구조로 확정.
> 실시간 스트리밍과 VLM 추론의 지연 격리를 위해 GStreamer `tee`로 분기하며, Branch A는 인코딩 스트림을 그대로 MediaMTX에 전달한다 (GPU 사용 없음). Branch B에서만 디코딩 후 AI 분석을 수행한다.
...
flowchart TD
    ...
    subgraph JETSON["Jetson Orin NX 16GB"]
        ...
        subgraph APP["App 컨테이너"]
            subgraph GST["GStreamer Pipeline"]
                ...
                subgraph BRANCH_A["Branch A — 실시간 스트리밍 (패스스루)"]
                    QA["queue\n(패스스루, 재인코딩 없음)"]
                end
    ...
    %% Branch A (패스스루: 디코딩 전 분기, 재인코딩 없음)
    TEE -->|Branch A| QA --> MTX
    MTX -->|"실시간 스트림"| WEB
    MTX -->|"실시간 스트림"| APP_MOB

    %% Branch B (디코딩 후 AI 분석)
    TEE -->|Branch B| QB --> DEC --> CONV --> SINK
    SINK -->|"프레임 Push"| RING
...
**Branch A — 실시간 스트리밍 (패스스루)**:
- **디코딩 전** tee에서 분기 → 인코딩된 스트림 그대로 MediaMTX에 전달 (재인코딩 없음)
- GPU 사용 없음. Branch B의 추론 지연과 완전히 격리

**Branch B — AI 분석**:
- tee 이후 `nvv4l2decoder`로 GPU 디코딩 수행
...
| OI-05 | ~~Tee 분리 파이프라인 구조 채택 여부~~ → **확정** | 디코딩 전 tee 패스스루 구조 채택. Branch A는 재인코딩 없이 MediaMTX로 포워딩. 구현 시 확인 |
...
| OI-08 | 컨테이너 간 통신 방식 | NanoLLM ↔ App, App ↔ API 서버 등 컨테이너 추가로 설계 범위 확장. 인터페이스(gRPC/REST/소켓) 설계 필요 |
...
- [ ] Docker 컨테이너 구성 (NanoLLM, MediaMTX, App, API 서버, DB)
- [ ] NanoLLM + VILA 동작 확인
