...
| OI-09 | 카메라 코덱 확인 (H.264 / H.265) | H.264를 기본 가정. H.265 전환 시 파이프라인 `rtph264depay/h264parse` → `rtph265depay/h265parse` 2단어 수정. 실 카메라 연결 시 확인 |
| OI-10 | ONVIF PTZ 카메라 제어 | 현재 파이프라인과 독립적으로 분리 개발 가능. PTZ 동작 중 VLM 오탐 가능성은 추후 결정 (UX 레벨 처리 또는 추론 억제 신호) |
