...
`streamer`는 두 프로세스다. 동반 프로세스(Python)가 부모로서 기동하여 MediaMTX를 자식 프로세스로 띄우고, 자식의 비정상 종료 시 재기동하며(간격은 1초에서 2배씩 상한 10초, 60초 이상 안정 가동 뒤에는 1초로 복귀), 자기 종료 시 자식을 먼저 종료한다(SIGTERM 후 10초 뒤 SIGKILL). 이 감독 방식은 `analyzer`가 VLM 자식 프로세스를 다루는 방식과 동일하다. 동반 프로세스는 그 밖에 기동 시 프로필 복원, PTZ 상태 폴링, PTZ 자동 순찰의 세 스레드를 가진다.
...
`recorder`는 uvicorn 프로세스 안에 세그먼트 기록용 GStreamer 파이프라인 스레드와 클립 결합 작업 스레드를 가진다. 하드웨어 상태는 상태 조회 요청 시점에 측정하며 상주 루프를 두지 않는다. ffmpeg는 클립 결합과 직접 녹화 때 1회 실행되는 자식 프로세스이며 감독 대상이 아니다. 클립 결합은 이벤트 통지 요청의 응답과 분리된 작업 스레드에서 수행하여, 결합이 오래 걸려도 통지 경로가 막히지 않게 한다.
...
- 최초 기동에서는 VLM 모델의 사전 컴파일이 수 분에서 수십 분 걸린다(SRS §3.2). 이 동안 `analyzer`의 HTTP 서버는 이미 기동되어 설정 저장과 분석 시작 요청을 접수하되 파이프라인은 모델 적재가 끝난 뒤에 시작하며(§7.3), 모델 전환 요청은 400으로 거부한다. 로그인·프로필·클립 조회 등 나머지 기능은 정상 제공한다. 컴포넌트 분리가 이 부분 가용성을 자연히 보장한다.
