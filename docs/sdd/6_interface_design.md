...
|경로|메서드|기능|인증|전달 대상|
|---|---|---|---|---|
|`/client/storage/{key}`|GET|클라이언트 데이터 문서 조회(`FR-059`). 없는 키는 빈 문서를 반환한다.|필요|`router` 자체|
|`/client/storage/{key}`|PUT|클라이언트 데이터 문서 저장(전체 교체). 크기 상한만 검증하고 내용은 해석하지 않는다.|필요|`router` 자체|

키는 `[a-z0-9_-]{1,64}`로 제한한다. 키의 의미(예: mewly의 `pet_profile`)는 ***Client app***이 정하며, 시스템은 관여하지 않는다.
...
- **액세스 토큰** — HMAC-SHA256 서명 JWT. 페이로드는 사용자 식별자, 만료 시각(발급 후 600초), 세대(epoch)다. ***Request router***가 발급과 검증을 모두 수행하며, 서명 비밀키는 이 컴포넌트만 보유한다(§2.3).
