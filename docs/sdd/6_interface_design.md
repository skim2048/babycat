...
|경로|메서드|기능|인증|전달 대상|
|---|---|---|---|---|
|`/api/login`|POST|로그인. 액세스·리프레시 토큰 발급.|불필요|`manager`|
|`/api/refresh`|POST|리프레시 토큰으로 액세스 토큰 갱신(회전).|불필요|`manager`|
|`/api/logout`|POST|토큰 폐기.|불필요|`manager`|
|`/api/change-password`|POST|비밀번호 변경(기존 토큰 전체 폐기).|필요|`manager`|
...
|경로|메서드|기능|인증|전달 대상|
|---|---|---|---|---|
|`/camera`|GET|비디오 소스 프로필 조회(비밀번호 마스킹).|필요|`controller`|
|`/camera`|POST|비디오 소스 프로필 적용.|필요|`controller`|
|`/ptz`|POST|팬·틸트·줌 이동/정지/홈 저장/홈 복귀.|필요|`controller`|
...
|`/analysis/start`|POST|장면 분석 시작/재시작(`FR-024`). `analyzer`·`controller`·`recorder`에 병렬 전달(SRS §2.3 (4)).|필요|셋 모두|
...
- **액세스 토큰** — HMAC-SHA256 서명 JWT. 페이로드는 사용자 식별자, 만료 시각(발급 후 600초), 세대(epoch)다. ***Account manager***가 발급하고 ***Request router***가 검증한다. 서명 비밀키는 두 컴포넌트에만 주입한다(§2.3).
- **리프레시 토큰** — 임의 난수 토큰. 원문은 클라이언트만 보관하고 서버는 해시만 저장하며, 만료는 발급 후 30일이다. 갱신마다 회전한다(`FR-045`).
- **폐기** — 로그아웃과 비밀번호 변경 시 ***Account manager***가 계정의 세대를 증가시킨다. ***Request router***는 매 인증마다 토큰의 세대를 ***Account manager***의 현재 세대와 대조하므로, 이전 세대의 액세스 토큰은 즉시 거부된다(`FR-003`·`FR-005`). 이 대조는 요청당 내부 HTTP 한 번이며, 소수 운영자 전제(SRS §5.2)에서 부담이 없다. 부하가 문제되면 짧은 TTL 캐시를 도입하되 그 TTL이 곧 폐기 지연 상한이 됨을 명시한다.
...
내부 호출은 컨테이너 네트워크의 HTTP이며 인증을 두지 않는다(§2.2). 내부 포트는 `manager` 8100, `controller` 8200, `analyzer` 8300, `recorder` 8400이며 어느 것도 호스트에 공개하지 않는다.

|제공자|경로|호출자|기능|
|---|---|---|---|
|`manager`|POST `/internal/login`·`/internal/refresh`·`/internal/logout`·`/internal/change-password`|`router`|§6.1 인증 기능의 실체|
|`manager`|GET `/internal/epoch`|`router`|현재 토큰 세대 조회(§6.2)|
|`controller`|GET·POST `/profile`|`router`|프로필 조회·적용|
|`controller`|POST `/ptz`|`router`|PTZ 명령|
|`controller`|POST `/activate`|`router`|저장된 프로필로 소스 재배포 지시(분석 시작의 일부)|
|`controller`|GET `/status`|`router`|PTZ 위치 상태(모니터링 합성용)|
...
|`streamer`|MediaMTX 제어 API(9997)|`controller`|RTSP 소스 설정|
|`streamer`|HLS(8888)·WHEP(8889)|`router`|재생 중계의 상류|
|`streamer`|RTSP(8554)|`analyzer`·`recorder`|재배포 스트림 수신|

이벤트 통지(`/notify`)의 본문은 매칭 키워드 목록, 장면 설명 텍스트, 발생 시각, 그리고 진단용 추론 시각 정보다. ***Event recorder***는 수신 즉시 응답하고 클립 결합은 작업 스레드에서 수행한다(§3.4). 통지가 유실되면 해당 이벤트는 기록되지 않으며, 이를 좁히는 재전송은 두지 않는다 — 추론 주기가 수 초이므로 지속되는 상황은 다음 추론에서 다시 판정된다.

## 6.4 스트리밍 인터페이스 (Streaming Interface)

### (1) RTSP 수신

***Video streamer***는 ***Source controller***가 설정한 RTSP URL로 ***Video source***에 접속하여 수신한다(`IF-002`). ***Video analyzer***와 ***Event recorder***는 각자 독립된 RTSP 연결로 재배포 스트림(`rtsp://streamer:8554/live`)을 소비한다. 접속 실패의 재시도(`FR-046`)는 간격을 늘려가며 무한 반복하되 실현 수단은 컴포넌트별로 다르다 — ***Event recorder***는 기록 파이프라인 재기동을 초기 1초에서 2배씩 상한 10초로, ***Video analyzer***는 워치독 재시작 간격을 기본 15초에서 2배씩 상한 60초로 늘리며, 프레임이 흐르기 시작하면 기본 간격으로 복귀한다. ***Source controller***의 제어 API 재시도는 상한 30초의 점진 재시도(`FR-015`)이며, 이후는 주기 점검(§7.5)이 이어받는다.
...
`FR-042`·`FR-043`의 실시간 제공은 ***Request router***의 합성으로 실현한다. ***Request router***는 `analyzer`의 SSE를 구독하여 추론·파이프라인 상태 변화를 즉시 받고, `recorder`와 `controller`의 `/status`를 주기(2초)로 수집하여, 세 출처를 하나의 평면 JSON 스냅숏으로 병합해 `/state` SSE로 내보낸다. 어느 출처가 응답하지 않으면 해당 필드 그룹을 결측으로 표시하고 나머지는 계속 전달한다 — 관측은 부분 실패에도 살아 있어야 한다(§2.1 목표 3).
