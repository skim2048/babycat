...
1. ***User***가 ***Client App***에 자격증명을 입력하면, ***Client App***은 ***Gateway***에게 로그인을 요청한다.
...
1. ***User***가 ***Client App***을 이용하여 ***Video Source*** 프로필, VLM, 프롬프트, 이벤트 키워드를 설정하면, ***Client App***은 이를 ***Gateway***에게 전달한다.
2. ***Gateway***는 이를 ***Engine***에게 전달한다.
3. ***Engine***은 전달받은 설정을 ***Storage***에 저장한다. 이 시점에 분석이 자동으로 시작되지는 않는다.

### (3) 분석 시작

1. ***User***가 ***Client App***을 이용하여 분석 시작을 요청하면, ***Client App***은 이를 ***Gateway***에게 전달한다.
2. ***Gateway***는 이 요청을 ***Engine***에게 전달한다.
3. ***Engine***은 ***Storage***로부터 ***Video Source*** 프로필을 읽어, 해당 ***Video Source***의 스트림을 받아오도록 ***Media***에게 요청한다.
4. ***Media***는 ***Video Source***에 접속하여 RTSP로 비디오 스트림을 수신하고, 이를 내부에 재배포한다.
5. ***Engine***은 ***Storage***로부터 VLM과 프롬프트를 읽어, ***Media***가 재배포하는 스트림을 입력으로 삼아 장면 분석 파이프라인을 기동한다.
...
1. ***User***가 ***Client App***을 이용하여 재생을 요청하면, ***Client App***은 ***Gateway***에게 스트림 접근 토큰을 요청한다.
...
1. ***User***가 ***Client App***을 이용하여 팬·틸트·줌을 조작하면, ***Client App***은 이를 ***Gateway***에게 전달한다.
2. ***Gateway***는 이를 ***Engine***에게 전달한다.
3. ***Engine***은 ONVIF로 ***Video Source***를 직접 제어한다.
4. ***Video Source***가 ONVIF를 지원하지 않거나 접근을 허용하지 않으면, 요청은 별도의 오류 없이 무시된다.

### (7) 저장된 클립과 이력 조회

1. ***User***가 ***Client App***을 이용하여 조건(키워드, 날짜)으로 조회를 요청하면, ***Client App***은 이를 ***Gateway***에게 전달한다.
2. ***Gateway***는 ***Storage***의 데이터베이스에서 해당하는 이력을 찾아 ***Client App***에게 전달한다.
3. ***User***가 ***Client App***을 이용하여 특정 클립의 재생이나 삭제를 요청하면, ***Client App***은 이를 ***Gateway***에게 전달한다.
4. ***Gateway***는 ***Storage***의 클립 파일을 읽어 전달하거나 삭제한다.
...
소규모 프로젝트 특성상, 날짜가 아닌 기능 단위로 단계를 나눈다. ***Gateway***를 단일 진입점으로 삼아 제공 기능의 여덟 기능군을 모두 구현한다(v1.0). 단, ***Video Source***는 H.264 RTSP 카메라 한 대로 한정한다. 첫 버전 출시 후, 차기 버전에서 아래 기능을 구현한다.
