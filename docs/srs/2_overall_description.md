# 2. 전체 설명 (Overall Description)

## 2.1 제품 조망 (Product Perspective)

<figure align="center">
  <img src="figs/2-1.drawio.svg" width="75%">
  <figcaption><em>그림 2-1. 제품 조망도</em></figcaption>
</figure>

- ***User*** : ***Client App***을 통해 `Babycat`을 조작하는 사람
- ***Client App*** : `Babycat` 사용자용 프론트엔드 앱
- ***Video Source*** : `Babycat`에 라이브 비디오를 제공하는 외부 소스

## 2.2 전체 시스템 구성 (Overall System Configuration)

<figure align="center">
  <img src="figs/2-2.drawio.svg" width="100%">
  <figcaption><em>그림 2-2. 전체 시스템 구성도</em></figcaption>
</figure>

- ***Gateway*** : 단일 외부 진입점, 사용자 인증 및 요청 처리·프록시
- ***Engine*** : VLM 추론, 이벤트 감지·기록, 프로필 관리, PTZ 제어 등
- ***Media*** : 라이브 비디오 스트림의 처리·분배(MediaMTX 기반)
- ***Storage*** : 설정 파일·비디오 클립·데이터베이스 저장 등

## 2.3 전체 동작 방식 (Overall Operation)

`Babycat`은 ***User***가 ***Client App***으로 시스템을 조작하는 **상호작용** 방식과, ***User***의 개입 없이 상시 반복되는 **자율분석** 방식으로 동작한다. 자율분석 표시가 없는 동작은 모두 상호작용 방식이다.

### (1) 사용자 인증

1. ***User***가 ***Client App***에 자격증명을 입력하면, ***Client App***은 ***Gateway***에 로그인을 요청한다.
2. ***Gateway***는 ***Storage***의 데이터베이스에서 계정을 조회하여 자격증명을 검증한다.
3. 검증에 성공하면 ***Gateway***는 액세스 토큰을 발급하여 ***Client App***에 전달한다.
4. ***Client App***은 이후의 모든 요청에 이 토큰을 실어 보낸다. ***Gateway***는 토큰이 없거나 유효하지 않은 요청을 거부한다.

### (2) 장면 분석 시작

1. ***User***가 ***Client App***에서 ***Video Source*** 접속 프로필, VLM, 프롬프트, 이벤트 키워드를 설정하면, ***Gateway***는 이를 ***Engine***에 전달한다.
2. ***Engine***은 전달받은 설정을 ***Storage***에 저장한다. 이 시점에 분석이 자동으로 시작되지는 않는다.
3. ***User***가 분석 시작을 요청하면, ***Gateway***는 이를 ***Engine***에 전달한다.
4. ***Engine***은 설정된 프로필의 ***Video Source***를 ***Media***에 스트림 소스로 등록한다.
5. ***Media***는 ***Video Source***에 접속하여 RTSP로 비디오 스트림을 수신하고, 이를 내부에 재배포한다.
6. ***Engine***은 ***Media***가 재배포하는 스트림을 입력으로 삼아 장면 분석 파이프라인을 기동한다.

### (3) 이벤트 감지와 기록 - 자율분석

1. ***Engine***은 파이프라인이 실행되는 동안 비디오 스트림에서 주기적으로 프레임을 추출하여 VLM에 입력하고, 장면을 설명하는 텍스트를 생성한다.
2. ***Engine***은 생성된 텍스트에 ***User***가 설정한 이벤트 키워드가 포함되어 있는지 검사한다.
3. 키워드가 포함되어 있으면, ***Engine***은 해당 상황을 이벤트 발생으로 판정한다.
4. ***Engine***은 그 구간의 비디오 클립과 메타데이터를 ***Storage***에 저장하고, 발생 이력을 데이터베이스에 기록한다.
5. ***Engine***은 가용 저장 공간이 임계치 이하로 떨어지면, 가장 오래된 클립과 이력부터 자동으로 삭제하여 공간을 확보한다.

### (4) 라이브 비디오 재생

1. ***User***가 ***Client App***에서 재생을 요청하면, ***Client App***은 ***Gateway***에 스트림 접근 토큰을 요청한다.
2. ***Gateway***는 토큰을 발급한다.
3. ***Client App***은 발급받은 토큰을 지니고 ***Media***에 직접 접속한다. 저지연을 위해 ***Gateway***를 경유하지 않는다.
4. ***Media***는 토큰을 검증한 뒤 라이브 비디오를 HLS 또는 WebRTC로 전달한다.

### (5) 비디오 소스 PTZ 제어

1. ***User***가 ***Client App***에서 팬·틸트·줌을 조작하면, ***Gateway***는 이를 ***Engine***에 전달한다.
2. ***Engine***은 ONVIF로 ***Video Source***를 직접 제어한다.
3. ***Video Source***가 ONVIF를 지원하지 않거나 접근을 허용하지 않으면, 요청은 별도의 오류 없이 무시된다.

### (6) 저장된 클립과 이력 조회

1. ***User***가 ***Client App***에서 조건(키워드·날짜)을 지정하여 조회를 요청하면, ***Gateway***는 ***Storage***의 데이터베이스에서 해당하는 이력을 찾아 ***Client App***에 전달한다.
2. ***User***가 특정 클립의 재생이나 삭제를 요청하면, ***Gateway***는 ***Storage***의 클립 파일을 읽어 전달하거나 삭제한다.

## 2.4 제공 기능 (Functions)

- **사용자 계정 인증 및 관리** — ***Client App***이 `Babycat`에 접근하려면 인증을 거쳐야 한다. 인증된 ***User***는 재로그인 없이 상태를 유지하며 자신의 비밀번호를 변경할 수 있다. 다수 계정이 필요하지 않다고 판단하여 계정의 추가·삭제 기능은 두지 않으며, `admin` 계정 하나만을 대상으로 한다.

- **비디오 소스 프로필 관리** — 프로필은 ***Video Source***에 접근하기 위한 정보의 집합으로, IP 주소·포트·스트림 경로·자격증명 등으로 구성된다. ***User***는 프로필을 등록·조회·수정할 수 있다. ***Video Source***에는 RTSP 카메라, 비디오 파일, USB 카메라, 미디어 서버 등 여러 유형이 있으나, 가장 대중적인 RTSP 카메라만을 대상으로 한다.

- **비디오 소스 PTZ 제어** — ***User***는 ***Video Source***의 팬·틸트·줌을 조작하고, 홈 위치를 저장하여 그리로 되돌릴 수 있다. ***Video Source***가 ONVIF를 지원하지 않거나 접근을 허용하지 않으면 요청은 별도의 오류 없이 무시된다.

- **라이브 스트리밍** — ***User***는 ***Video Source***의 라이브 비디오를 재생할 수 있다. 비디오는 HLS 또는 WebRTC로 전달되며, 재생하려면 ***Gateway***가 발급한 인증 토큰이 있어야 한다.

- **장면 분석 및 이벤트 기록** — `Babycat`의 핵심 기능군이다. 설정된 VLM과 ***User***가 입력한 프롬프트로 장면을 분석하고, ***User***가 설정한 키워드에 해당하는 상황을 이벤트로 감지하여 그 구간의 비디오 클립과 발생 이력을 자동으로 저장한다.

- **이벤트 발생 이력 관리** — ***User***는 저장된 이벤트 발생 이력을 조건(키워드·날짜)으로 조회하고 삭제할 수 있다.

- **비디오 클립 관리** — ***User***는 저장된 비디오 클립을 조건(키워드·날짜)으로 조회·재생·삭제할 수 있다.

- **시스템 실시간 모니터링** — ***User***는 VLM 분석 과정과 하드웨어 상태(온도·메모리 등)를 실시간으로 확인할 수 있다.

## 2.5 사용자 계층과 특징 (User Classes and Characteristics)

- 연구자
  - VLM이 자기 분야의 장면을 알아볼 수 있는지 시험해 보려는 사람이다.
  - 프롬프트와 이벤트 키워드를 바꿔가며 감지 결과가 어떻게 달라지는지 비교한다.
  - 쌓인 클립과 발생 이력을 훑어 VLM의 판정이 쓸 만한지 가늠한다.

- 개발자
  - 자신의 감시 서비스 및 앱을 만들려는 사람이다.
  - HTTP API와 라이브 스트림을 가져다 자신의 ***Client App***을 붙인다.
  - Jetson Board와 Docker를 다룰 줄 안다.

- 현장 관리자
  - 영상을 외부로 내보낼 수 없는 현장에서 카메라를 지켜봐야 하는 사람이다.
  - 감시할 상황을 키워드로 걸어두고, 이벤트가 잡히면 그 구간의 클립만 확인한다.
  - 화면을 종일 지켜보지 않는다.

## 2.6 가정과 종속 관계 (Assumptions and Dependencies)

- 키워드 매칭 방식을 이용해, 목적 이벤트를 유의미한 수준으로 감지할 수 있다고 가정한다.
- 개인영상정보 관련 법규가 `Babycat`의 운용을 제약하지 않는다고 가정한다.
- 메모리가 16GB 이상의 Jetson Board에서 구동된다고 가정한다.
- 전송 계층 암호화를 적용하지 않으므로, 신뢰할 수 있는 내부 네트워크(LAN 또는 VPN) 안에서만 운영된다고 가정한다.
- NanoLLM이 향후에도 Jetson Platform 지원과 프로젝트 호환성을 유지한다고 가정한다.
- 하드웨어 비디오 디코더/인코더를 갖춘 Jetson Module(Orin NX 이상)이 필요하다.
- JetPack 6.2.1, Docker 29.1.3, NVIDIA Container Toolkit 1.16.2 위에서 동작한다.
- 현장에서 이미지를 빌드하므로, ***Engine***의 베이스 이미지(`dustynv/nano_llm`)를 Docker Hub에서 받을 수 있어야 한다.
- ***Media***는 MediaMTX를 사용하나, 그 버전이 아직 고정되어 있지 않다.
- ***Video Source***는 H.264로 인코딩된 비디오 스트림을 제공해야 한다.
- PTZ 제어는 ***Video Source***의 ONVIF 지원 여부에 달려 있다.

## 2.7 단계별 요구사항 (Apportioning of Requirements)

소규모 프로젝트 특성상, 날짜가 아닌 기능 단위로 단계를 나눈다. ***Gateway***를 단일 진입점으로 삼아 제공 기능의 여덟 기능군을 모두 구현한다(v1.0). 단, ***Video Source***는 H.264 RTSP 카메라 한 대로 한정한다. 첫 버전 출시 후, 다음 버전에서 아래 기능을 구현한다.

- 다중 카메라 지원 : 단일 카메라를 전제한 파이프라인 구조와 프로필 데이터 모델을 재설계해야 한다.
- RTSP 외 ***Video Source*** 유형 지원 : 소스 유형과 프로필 데이터 모델을 소스별로 나누어야 한다.
- H.264 외 코덱 지원 : GStreamer 파이프라인에서 코덱 처리를 추상화해야 한다.
- 다수 계정 지원 : 단일 `admin` 계정을 전제한 인증 구조에 계정 관리와 권한 구분을 더해야 한다.
- 이벤트 푸시 알림 : 외부 푸시 서비스(FCM 등) 연동으로 외부 시스템 구성이 바뀌고, ***Gateway***에 디바이스 토큰 관리가 추가된다.

장기 비디오 트렌드 분석과 Jetson 외 환경은 `Babycat` 개발 범위를 벗어난다.

## 2.8 하위 호환성 (Backward Compatibility)

이 시스템은 첫 버전이기 때문에 아직 하위 호환성을 고려할 필요가 없다.
