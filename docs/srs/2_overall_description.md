...
`Babycat`은 Jetson Board에서 동작하는 독립 실행형 백엔드이며, ***Video Source***로부터 비디오 스트림을 수신하여 VLM으로 분석하고 ***Client App***의 요청에 맞는 서비스를 제공한다.
...
- ***MediaMTX Server***
  - `Babycat`의 미디어 서버
  - ***Video Source***의 라이브 비디오 자원을 적절히 분배
...
## 2.4 제품 주요 기능 (Product Functions)
...
|5-4|설정된 키워드에 해당하는 이벤트를 자동으로 감지하고 비디오 클립을 저장한다|

5-1부터 5-3은 사용자가 분석 조건을 설정하는 기능이며, 5-4는 그 설정에 따라 사용자의 개입 없이 시스템이 수행하는 자동 동작이다.
...
## 2.6 가정과 종속관계 (Assumptions and Dependencies)
