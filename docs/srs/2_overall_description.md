...
- ***Request router*** : 단일 진입점으로 들어온 모든 요청의 수신자. 계정을 인증·관리하며, 요청에 실린 액세스 토큰을 검증하여 라우팅한다. 외부 노출과 TLS 종단의 배치는 설계가 정한다(`NFR-016`).
...
1. ***User***가 팬·틸트 제어를 요청하면, ***Client app***은 이 요청을 ***Request router***에게 전달한다.
...
- **비디오 소스 PTZ 제어** (§7.3) — ***User***는 ***Video source***의 팬·틸트를 조작하고, 현재 위치를 프리셋 슬롯에 저장하여 그 위치로 이동시키거나 저장된 프리셋을 자동으로 순회시킬 수 있다. ***Video source***가 ONVIF를 지원하지 않거나 접근을 허용하지 않으면 시스템은 요청을 별도의 오류 없이 무시한다.
...
- **추론 이력 및 집계** (§7.9) — 시스템은 이벤트 판정과 무관하게 모든 추론을 보존하며, ***User***는 라벨·날짜 조건의 조회와 버킷 단위 집계로 하루 단위 행동 요약의 원자료를 얻을 수 있다.
- **클라이언트 데이터 보존** (§7.10) — ***Client app***은 자기 데이터를 계정별·키별 문서로 서버에 저장·조회할 수 있다. 시스템은 내용을 해석하지 않으며, 데이터는 클라이언트의 재설치나 기기 교체에도 보존된다.
...
소규모 프로젝트 특성상, 날짜가 아닌 기능 단위로 단계를 나눈다. ***Request router***를 단일 진입점으로 하여 제공 기능(§2.4)의 기능군 전체를 구현한다(v1.0). 단, ***Video source***는 H.264 RTSP 카메라 한 대로 한정한다. 첫 버전 출시 후, 차기 버전에서 아래 기능을 구현한다.
