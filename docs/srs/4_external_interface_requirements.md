...
본 장은 `Babycat` 경계를 넘는 외부 인터페이스를 정의한다. 내부 컴포넌트 간 인터페이스(***Gateway*** ↔ ***Engine*** 등)는 설계 문서의 범위로 한다. 외부 인터페이스에는 `IF` 식별자를 부여한다.
...
다음 엔드포인트는 재설계에서 ***Gateway***로 통합해야 하는 대상이며, 경로와 명세는 작성을 보류한다.

|기능|현재 위치|비고|
|---|---|---|
|PTZ 제어|***Engine*** 직접 노출|단일 진입점 원칙에 따라 ***Gateway*** 경유로 변경.|
|VLM 프롬프트, 이벤트 키워드 설정|***Engine*** 직접 노출|장면 분석 설정에 해당하는 경로.|
