...
- `FR-047` 시스템은 사용자가 올바른 자격증명으로 로그인하는 시점에, 1계정 1로그인 원칙을 지키기 위해 해당 계정의 기존 로그인 세션을 즉시 무효화해야 한다.
...
- `FR-016` 시스템은 사용자가 ***Client app***을 통해 요청할 때, ONVIF 표준 프로토콜을 사용하여 ***Video source***의 팬·틸트·줌 연속 이동 명령을 수행해야 한다.
  - PTZ 제어는 라이브 스트리밍이 진행 중일 때 가능하다(`FR-048`). 스트리밍 종료는 ***Video source***와의 PTZ 연결도 함께 끝낸다(`FR-049`).
...
- `FR-048` 시스템은 사용자가 ***Client app***을 통해 요청할 때, 등록된 프로필로 ***Video source***에 접속하여 라이브 스트리밍을 시작하는 기능을 제공해야 한다.
  - 프로필의 등록은 스트리밍 시작으로 이어지지 않으며, 시작 시점의 등록 프로필이 접속에 적용된다.
  - 시작 요청은 재시작을 겸한다.
- `FR-049` 시스템은 사용자가 ***Client app***을 통해 라이브 스트리밍 종료를 요청하는 시점에, ***Video source*** 연결을 해제하고 진행 중인 장면 분석과 이벤트 대비 버퍼링을 함께 정지해야 한다.
  - 장애로 인한 스트림 단절은 종료가 아니며, 접속 재시도(`FR-046`)로 수복한다.
...
- `FR-023` (삭제) 스트림 접근 토큰 요구사항은 단일 진입점 결정에 따라 삭제되었다. 번호는 재사용하지 않는다.

## 7.5 장면 분석 및 이벤트 기록 (Scene Analysis and Event Recording)

- `FR-024` 시스템은 사용자가 ***Client app***을 통해 요청할 때, 라이브 비디오 내 이벤트를 감지하기 위해 기설정된 프롬프트와 이벤트 키워드를 기반으로 장면 분석 파이프라인을 시작하거나 재시작하는 기능을 제공해야 한다.
- `FR-050` 시스템은 라이브 스트리밍이 진행 중이 아닐 때 장면 분석 시작 요청이 발생할 경우, 해당 요청을 거부해야 한다.
- `FR-051` 시스템은 사용자가 ***Client app***을 통해 요청할 때, 라이브 스트리밍을 유지한 채 장면 분석과 이벤트 대비 버퍼링만 정지하는 기능을 제공해야 한다.
