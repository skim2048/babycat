...
본 기능군의 프로필은 반려동물의 신상 정보이며, 비디오 소스 접속 정보인 §7.2의 프로필과 무관하다.

- `FR-059` 시스템은 사용자가 ***Client app***을 통해 요청할 때, 클라이언트의 재설치나 기기 교체에도 정보를 보존하기 위해 반려동물 프로필(이름, 견종, 생일, 사진, 특징)을 서버에 저장하고 조회하는 기능을 제공해야 한다.
  - 프로필은 계정에 속하며 계정당 하나이다. 저장은 프로필 전체를 교체한다.
  - 사진은 ***Client app***이 축소한 JPEG이며, 시스템은 저장 시 항목별 크기 상한을 검증한다.
