...
- `FR-001` 시스템은 사용자가 ***Client app***을 통해 올바른 자격증명으로 로그인하는 시점에, 외부 접근 권한 제어를 목적으로 그 ***Client app***에게 액세스 토큰(Access token)을 발급해야 한다.
  ...
  - 액세스 토큰의 만료 시각은 발급 시점을 기준으로 600초(10분) 후로 설정한다. 600초는 토큰 탈취 시의 노출 시간을 제한하면서 갱신 왕복이 잦지 않은 절충값이다.
  - 만료된 액세스 토큰의 요청은 401과 만료 사유로 거부한다. 리프레시 토큰이 없는 ***Client app***은 이 응답으로 세션 종료를 인지한다.
- `FR-047` 시스템은 사용자가 올바른 자격증명으로 로그인하는 시점에, 1계정 1로그인 원칙을 지키기 위해 그 계정의 기존 로그인 세션을 즉시 무효화해야 한다.
  ...
  - 대체된 세션의 요청은 401과 폐기 사유로 거부한다. ***Client app***은 만료와 구분되는 이 사유로 대체 사실을 사용자에게 알린다.
- `FR-002` 시스템은 사용자가 ***Client app***을 통해 올바른 자격증명으로 로그인하며 로그인 유지를 요청할 때, 잦은 재로그인 방지를 목적으로 액세스 토큰과 함께 리프레시 토큰(Refresh token)을 발급해야 한다.
  ...
  - 리프레시 토큰의 만료 시각은 발급 시점을 기준으로 2,592,000초(30일) 후로 설정한다. 30일은 로그인 유지의 관례적 기간이다.
  - 만료된 리프레시 토큰의 갱신 요청은 401로 거부한다.
...
- `FR-007` 시스템은 특정 사용자 계정의 로그인이 연속 10회 실패하는 시점에, 무차별 대입 공격(Brute-force attack)을 방어하기 위해 그 계정의 로그인을 30분(1,800초) 동안 차단해야 한다.
  - 차단을 일으키는 10회째 실패 응답부터 차단 응답(429)을 반환한다.
  - 10회·30분은 무차별 대입을 실질적으로 막으면서 정당한 사용자의 입력 오류를 허용하는 관례값이다.
...
- `FR-011` (삭제) 프로필 검증 요구사항은 등록(`FR-009`)의 일부로 흡수되어 삭제되었다. 번호는 재사용하지 않는다.
...
본 기능군은 ***Video source***가 ONVIF PTZ 기능을 지원하는 환경에서 팬·틸트 원격 제어를 수행하기 위해 적용한다. 대상 카메라에 광학 줌이 없으므로 줌 제어는 범위 밖이다.

- `FR-016` 시스템은 사용자가 ***Client app***을 통해 요청할 때, ONVIF 표준 프로토콜을 사용하여 ***Video source***의 팬·틸트 연속 이동 및 지정 좌표로의 절대 이동 명령을 수행해야 한다.
  - PTZ 제어는 라이브 스트리밍이 진행 중일 때 가능하다(`FR-048`). 스트리밍 종료는 ***Video source***와 맺은 PTZ 연결도 함께 끝낸다(`FR-049`).
- `FR-017` 시스템은 사용자가 ***Client app***을 통해 요청할 때, 진행 중인 팬·틸트 이동을 즉시 정지시켜야 한다.
- `FR-018` 시스템은 사용자가 ***Client app***을 통해 요청할 때, 현재의 팬·틸트 위치를 프리셋 슬롯(1–4)에 저장하도록 지원해야 한다. 슬롯 수 4는 단일 카메라의 감시 지점을 담기에 충분하다고 본 값이다.
...
- `FR-030` 시스템은 이벤트 발생을 판정한 시점에, 그 구간의 비디오 클립(mp4 파일)과 사이드카 메타데이터(json 파일: 발생 시각, 매칭 키워드, 장면 설명 텍스트 포함)를 한 쌍으로 결합하여 저장해야 한다.
  - 직전 기록으로부터 일정 시간(기본 30초) 안에 내려진 연속 판정은 같은 이벤트의 지속으로 보아 묶음 처리하며, 새 클립과 발생 이력(`FR-031`)을 만들지 않는다. 30초는 추론 주기(수 초)의 수 배로, 한 상황의 연속 판정을 한 이벤트로 묶기에 충분한 값이다.
...
본 기능군은 ***Client app***이 하루 단위 행동 요약(상태 점유율의 시계열과 그 편차)을 산출할 수 있도록, 이벤트 판정과 별개로 모든 추론을 보존·집계하는 기능을 규정한다. 시스템은 클라이언트가 정의한 라벨이 언제·얼마나 발생했는가까지만 다루고, 라벨의 의미는 해석하지 않는다.

- `FR-053` 시스템은 비디오 분석 파이프라인이 실행되는 동안, 이벤트 판정 여부와 무관하게 모든 추론을 추론 이력으로 기록해야 한다.
  - 레코드는 판정 시각, 장면 설명 텍스트 원문, 매치된 상태 라벨 목록, 적용된 프리셋 식별자, 사용 모델, 추론 소요 시간으로 구성한다.
  - 원문 보존은 라벨 어휘가 변경되어도 과거 이력을 다시 매치할 수 있게 하기 위한 요구이다.
  - 추론 이력의 수명은 비디오 클립과 독립이며, 클립 자동 삭제(`FR-033`)는 추론 이력을 삭제하지 않는다. 시스템은 설정된 보존 기간(기본 90일)을 초과한 레코드를 자동 삭제한다.
- `FR-054` 시스템은 사용자가 ***Client app***을 통해 요청할 때, 상태 라벨 어휘를 설정하는 기능을 제공해야 한다.
  - 어휘는 라벨별 동의어 목록의 집합이며, 시스템은 라벨과 동의어를 의미 해석 없이 문자열로만 취급한다.
  - 시스템은 장면 설명 텍스트에 라벨의 동의어 중 하나가 대소문자 구분 없이 부분 일치로 포함될 때 그 라벨의 발생으로 판정한다. 이 판정은 이벤트 키워드 매칭(`FR-029`)과 같은 방식이되 이벤트 판정과는 독립적으로 수행한다.
  - 어휘의 기본값은 빈 값이며, 이때 추론 이력의 라벨 목록은 빈 값으로 기록된다.
- `FR-055` 시스템은 시간 구간별 프리셋이 설정되어 있는 동안, 추론을 시작하는 시점의 현지 시각이 속한 구간의 프롬프트와 라벨 어휘를 그 추론에 적용해야 한다.
  - 프리셋은 식별자, 시작 시각, 종료 시각, 프롬프트(선택), 라벨 어휘(선택)로 구성하며, 생략된 항목에는 기본 설정(`FR-026`, `FR-054`)을 적용한다.
  - 시작 시각이 종료 시각과 같거나 늦은 구간은 자정을 넘는 구간으로 해석한다. 어느 구간에도 속하지 않는 시각에는 기본 설정을 적용하고, 구간이 겹치면 목록 순서가 앞선 프리셋을 우선한다.
  - 프리셋의 전환은 추론 사이에서만 일어나며, 하나의 추론에는 하나의 프리셋만 적용된다.
  - 프리셋과 라벨 어휘 설정은 재기동 후 복원되고(`FR-014`의 연장), 저장만으로 분석을 시작하지 않는다(`FR-025`).
- `FR-056` 시스템은 사용자가 ***Client app***을 통해 라벨 및 날짜 조건을 입력할 때, 추론 이력 조회 기능을 제공해야 한다.
  - 라벨 조건은 레코드의 라벨 목록에 대한 정확 일치로 판정한다.
- `FR-057` 시스템은 사용자가 ***Client app***을 통해 조회 구간과 버킷 크기(시간 또는 일)를 지정할 때, 추론 이력의 라벨별 발생 수 집계를 제공해야 한다.
  - 응답의 각 버킷은 라벨별 발생 수와 함께 그 버킷의 전체 추론 수를 포함해야 한다. 추론 주기가 장비와 설정에 따라 가변이므로, 전체 추론 수를 분모로 하는 정규화 없이는 버킷 간 점유율 비교가 왜곡되기 때문이다.
  - 조회 구간 안의 빈 버킷도 0 값으로 응답에 포함한다.
- `FR-058` 시스템은 추론 시작 간격의 하한이 설정되어 있는 동안, 하한에 미달하는 간격의 추론 시작 신호를 건너뛰어야 한다.
  - 하한의 기본값은 0이며, 이때 추론 주기는 하드웨어의 추론 소요 시간이 결정한다.

## 7.10 클라이언트 데이터 보존 (Client Data Persistence)

본 기능군은 ***Client app***이 자기 데이터(예: 사용자 설정, 클라이언트 고유의 도메인 정보)를 클라이언트의 재설치나 기기 교체에도 잃지 않도록 서버에 보존하는 기능을 규정한다. 시스템은 데이터의 내용을 해석하지 않으며, 도메인 지식은 ***Client app***에 남는다.

- `FR-059` 시스템은 사용자가 ***Client app***을 통해 요청할 때, 계정별·키별 데이터 문서를 저장하고 조회하는 기능을 제공해야 한다.
  - 문서는 계정과 키의 쌍마다 하나이며, 저장은 문서 전체를 교체한다.
  - 시스템은 문서의 크기 상한만 검증하고 내용은 검증·해석하지 않는다.
  - 26-08-24, 반려동물 프로필 전용 기능으로 도입했던 것을 범용 기능으로 개정함 — 도메인 개념이 백엔드의 범용성을 해치지 않아야 한다.
