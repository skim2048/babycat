...
- `FR-015` 시스템은 프로필을 적용하는 시점에 ***Video streamer*** 컴포넌트가 아직 준비되지 않은 경우, 컨테이너 간 기동 순서 불일치를 극복하기 위해 연결을 점진적으로 재시도해야 한다. (가용성 요구사항은 `NFR-019`를 따른다.)
...
- `FR-021` 시스템은 인증된 사용자가 ***Client app***을 통해 재생을 요청할 때, ***Video streamer*** 컴포넌트를 거쳐 ***Video source***의 라이브 비디오 스트림을 직접 전달해야 한다. (접근 통제는 `FR-023`에 의거하며, 저지연을 위해 ***Request router***를 경유하지 않는다.)
- `FR-022` 시스템은 사용자의 다양한 네트워크 환경 및 지연 시간 요구사항을 충족하기 위해, 라이브 비디오 스트림을 HLS(고호환성 목적) 및 WebRTC(저지연 목적)의 두 가지 방식으로 제공해야 한다.
- `FR-023` ***Video streamer*** 컴포넌트는 미디어 스트림 재생 요청을 수신할 때, 비인가자의 직접 접근을 차단하기 위해 ***Request router***가 발급한 유효 토큰을 보유한 요청에 대해서만 스트림을 제공해야 한다.
  - 스트림 접근 토큰은 액세스 토큰과 동일한 접근 통제 목적을 가지므로, 그 유효 조건을 따른다.
  - 다만 ***Video streamer***가 ***Request router***를 경유하지 않고 검증하므로, 로그아웃 및 비밀번호 변경에 따른 즉시 폐기(`FR-003`, `FR-005`)는 보장되지 않는다. 폐기 지연의 허용 범위와 이를 좁히기 위한 방식은 작성을 보류한다.

## 7.5 장면 분석 및 이벤트 기록 (Scene Analysis and Event Recording)

- `FR-024` 시스템은 사용자가 ***Client app***을 통해 요청할 때, 라이브 비디오 내 이벤트를 감지하기 위해 기설정된 프로필, 프롬프트, 이벤트 키워드를 기반으로 장면 분석 파이프라인을 시작하거나 재시작하는 기능을 제공해야 한다.
- `FR-025` 시스템은 프로필, 프롬프트, 이벤트 키워드 설정이 저장되는 시점에, 사용자가 의도하지 않은 시점의 분석 활성화를 방지하기 위해 장면 분석 파이프라인을 자동으로 시작하거나 재시작하지 않아야 한다. (활성화는 `FR-024`의 명시적 요청으로만 수행한다.)
