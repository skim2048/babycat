...
- `FR-001` 시스템은 사용자가 ***Client App***을 통해 올바른 자격증명으로 로그인하는 시점에, 외부 접근 권한 제어를 목적으로 해당 ***Client App***에게 액세스 토큰(Access token)을 발급해야 한다.
...
- `FR-003` 시스템은 사용자가 로그아웃하는 즉시, 해당 ***Client App***에게 부여된 액세스 토큰과 리프레시 토큰을 무효화해야 한다.
...
- `FR-015` 시스템은 프로필을 적용하는 시점에 ***Media*** 컴포넌트가 아직 준비되지 않은 경우, 컨테이너 간 기동 순서 불일치를 극복하기 위해 연결을 점진적으로 재시도해야 한다. (가용성 요구사항은 `NFR-019`를 따른다.)
...
- `FR-023` ***Media*** 컴포넌트는 미디어 스트림 재생 요청을 수신할 때, 비인가자의 직접 접근을 차단하기 위해 ***Gateway***가 발급한 유효 토큰을 보유한 요청에 대해서만 스트림을 제공해야 한다.
  - 스트림 접근 토큰은 액세스 토큰과 동일한 접근 통제 목적을 가지므로, 그 유효 조건을 따른다.
  - 다만 ***Media***가 ***Gateway***를 경유하지 않고 검증하므로, 로그아웃 및 비밀번호 변경에 따른 즉시 폐기(`FR-003`, `FR-005`)는 보장되지 않는다. 폐기 지연의 허용 범위와 이를 좁히기 위한 방식은 작성을 보류한다.
