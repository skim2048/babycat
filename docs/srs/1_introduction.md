...
- 프로젝트가 반드시 충족해야 할 요구사항을 설계와 구현에 앞서 확정한다.
- 확정된 요구사항을 설계 및 구현의 기준이자 검증의 근거로 활용한다.
- 각 요구사항에 식별자(§1.3 참조)를 부여하여, 매칭되는 산출물과 테스트를 추적한다.

## 1.2 프로젝트 범위 (Project Scope)

- 프로젝트명은 `Babycat`이다.
- `Babycat`은 비디오 분석 기술이 요구되는 분야에 VLM을 도입하기 위한 범용 백엔드이다.
- `Babycat`의 대상 도메인에 대한 VLM 적용 가능성을 검증하는 것이다.
- `Babycat`의 프로덕션 활용은 이 검증이 완료된 경우에만 신뢰할 수 있다.
- `Babycat`은 키워드 매칭 방식의 이벤트 탐지 기능을 제공한다.
- `Babycat`은 이벤트가 탐지된 구간을 비디오 클립으로 자동 저장하는 기능을 제공한다.
...
요구사항의 우선순위를 아래와 같이 표기한다. 표기가 없는 요구사항은 `P1`로 간주한다.
...
### 설계자
  ...
  - 이후 2.3에서 전체 동작 방식을, 2.7에서 버전별 기능 범위를 확인하고, Section 4에서 외부 인터페이스 요구사항을 살펴본다.

### 개발자
  - 구현에 필요한 요구사항을 파악하는 데 집중한다.
  - 1.3에서 요구사항 식별자 체계를 숙지하고, 1.4에서 용어를 확인한 뒤, Section 3에서 개발 및 운영 환경을 파악한다.
  - 이후 Section 4에서 인터페이스 명세를 검토하고, Section 7에서 기능 요구사항을 확인한다.

### 엔지니어
  - 시스템 배포 및 운영에 필요한 정보를 파악하는 데 집중한다.
  - Section 3에서 운영·배포·개발 환경을 확인하고, Section 5에서 성능 요구사항, Section 6에서 비기능 요구사항을 검토한다.
