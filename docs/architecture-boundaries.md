...
- Flow: camera RTSP is routed through MediaMTX for live playback, while `app` serves MJPEG debug frames and SSE runtime snapshots directly; `web` consumes both paths.
- Watch for: auth differences, `HOST_IP` and WebRTC wiring, SSE field changes, MJPEG query-token usage.
