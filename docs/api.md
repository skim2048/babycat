...
클립 파일은 이벤트 감지 시 App 컨테이너가 MediaMTX RTSP를 1초 단위 롤링 세그먼트로 유지하다가, 이벤트 전후 구간을 하나의 mp4로 확정한 파일이다. 현재 기본 창은 `이벤트 이전 2초 + 이후 5초`이며, 저장 구조는 `/data/{YYYY}/{MM}/{YYYYMMDD}_{HHMMSS}_{ms}.mp4`다. 동일 이름의 `.json` 파일은 트리거 메타데이터(timestamp/keywords/vlm_text)와 진단 필드(event_time_ms, ffmpeg_started_at_ms, start_delay_ms, ffmpeg_elapsed_ms, clip_size_bytes, clip_duration_s, segment_window_start_ms, segment_window_end_ms, selected_segment_count 등)를 담을 수 있다. 파일명의 날짜/시간은 컨테이너 로컬 timezone 기준이며, 현재 운영 기본값은 `Asia/Seoul`이다.
