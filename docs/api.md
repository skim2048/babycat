...
| `stream_protocol` | string | 선택 | Web 재생 선호 프로토콜. 현재 허용값은 `hls`, `webrtc`이며, 그 외 값은 저장 시 `hls`로 정규화됩니다. 대시보드는 연결 실패 시 이 선호값과 다른 방식으로 한 번 폴백할 수 있습니다. MediaMTX `sourceProtocol`은 별개로 항상 `tcp`로 고정 전송됩니다 |
