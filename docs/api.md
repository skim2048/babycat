...
클립 파일은 기본적으로 이벤트 감지 시 App 컨테이너가 ffmpeg로 MediaMTX RTSP 스트림에서 약 5초간 직접 녹화한 mp4 파일이다. 저장 구조는 `/data/{YYYY}/{MM}/{YYYYMMDD}_{HHMMSS}_{ms}.mp4`다. 동일 이름의 `.json` 파일은 트리거 메타데이터(timestamp/keywords/vlm_text)와 진단 필드(event_time_ms, ffmpeg_started_at_ms, start_delay_ms, ffmpeg_elapsed_ms, clip_size_bytes, clip_duration_s 등)를 담을 수 있다. 파일명의 날짜/시간은 컨테이너 로컬 timezone 기준이며, 현재 운영 기본값은 `Asia/Seoul`이다.

실험용 옵션으로 `TRIGGER_ROLLOVER_ENABLED=1`을 주면 App이 1초 단위 롤링 세그먼트를 유지하고, 이벤트 전후 구간을 하나의 mp4로 확정하려 시도한다. 이 경로가 실패하면 현재 구현은 direct RTSP 녹화로 자동 fallback한다. rollover 경로를 사용할 때는 sidecar에 `segment_window_start_ms`, `segment_window_end_ms`, `selected_segment_count` 같은 추가 필드가 들어갈 수 있다.
