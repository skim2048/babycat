...
클립 파일은 이벤트 감지 시 App 컨테이너가 ffmpeg로 RTSP 스트림에서 약 5초간 녹화한 mp4 파일이다. 저장 구조는 `/data/{YYYY}/{MM}/{YYYYMMDD}_{HHMMSS}_{ms}.mp4`이며, 동일 이름의 `.json` 파일이 트리거 메타데이터(timestamp/keywords/vlm_text)와 진단 필드(event_time_ms, ffmpeg_started_at_ms, start_delay_ms, ffmpeg_elapsed_ms, clip_size_bytes, clip_duration_s 등)를 담을 수 있다. 파일명의 날짜/시간은 컨테이너 로컬 timezone 기준이며, 현재 운영 기본값은 `Asia/Seoul`이다.
