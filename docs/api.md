...
HLS 대비 지연이 낮다. 프론트엔드는 HLS/WebRTC 토글을 제공한다 (연결 실패 시 상호 폴백).
...
카메라 IP/자격증명/스트림 경로를 관리한다. 동일한 엔드포인트 두 개가 존재한다:
...
카메라 프로파일을 적용한다. 변경 후 App 컨테이너는 RTSP 파이프라인을 자동 재시작한다.
...
|---|---|---|---|
| `ip` | string | 필수 | 카메라 IP |
| `username` | string | 필수 | ONVIF/RTSP 사용자명 |
| `password` | string | 선택 | 생략 시 기존 비밀번호 유지 |
| `rtsp_port` | int | 선택 | 기본 554 |
| `onvif_port` | int | 선택 | 기본 2020 |
| `stream_path` | string | 선택 | RTSP path. 선행 슬래시 없이 전달 (기본값 `stream1`, URL 구성 시 `/` 뒤에 붙음) |
| `stream_protocol` | string | 선택 | 프로파일에 저장만 됩니다. MediaMTX `sourceProtocol`은 현재 구현상 항상 `tcp`로 고정 전송됩니다 |
...
클립 파일은 이벤트 감지 시 App 컨테이너가 ffmpeg로 RTSP 스트림에서 5초간 녹화한 mp4 파일이다. 저장 구조는 `/data/{YYYY}/{MM}/{YYYYMMDD}_{HHMMSS}_{ms}.mp4`이며, 동일 이름의 `.json` 파일이 트리거 메타데이터(timestamp/keywords/vlm_text)를 담는다.
...
| 이름 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `q` | string | 선택 | 파일명 키워드 필터 (부분 일치, 대소문자 무관) |
| `limit` | int | 선택 | 최대 반환 개수. 기본값 100 (>=1) |
