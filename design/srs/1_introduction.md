...
- Babycat의 일차 목표는 대상 도메인에 대한 VLM 적용 가능성을 검증하는 것이다. 프로덕션 활용은 이 검증이 완료된 경우를 전제한다.
...
버전은 v1.0의 내용에는 표기하지 않고, 이후 버전에서 지원할 내용에만 `(v2.0)`과 같이 표기한다.

다이어그램에서 실선 화살표는 상시 연결을, 점선 화살표는 조건부 연결을 나타낸다.
...
### 설계자
시스템의 범위와 구조를 파악하는 데 집중한다. 1.1과 1.2에서 문서의 목적과 제품 범위를 확인한 뒤, 2.1과 2.2에서 시스템 구조와 컴포넌트 구성을 검토한다. 이후 2.3에서 전체 동작 방식을, 2.7에서 버전별 기능 범위를 확인하고, Section 4에서 외부 인터페이스 요구사항을 살펴본다.
...
최종 산출물은 Babycat이라는 명칭의 독립 실행형 백엔드 소프트웨어이며, Docker 이미지 형태로 제공된다.
